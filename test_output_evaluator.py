...
def test_good_response_scores_high(output_evaluator):
    
    case = Case(name="good", input="Find flights NYC to London")
    report = output_evaluator.evaluate(case, "BA117 at 7PM ($450), DL1 at 9:30PM ($520)")
    assert report.success

def test_vague_response_scores_low(output_evaluator):
    
    case = Case(name="vague", input="Find flights NYC to London")
    report = output_evaluator.evaluate(case, "There are several flights available. Prices vary.")
